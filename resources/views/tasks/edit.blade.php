<x-app-layout>
    ...
    <div class="min-h-screen bg-[#242b3d] text-white p-6 md:p-10" style="font-family: 'Inter', sans-serif;">
        <div class="max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-8">
            ...
            <!-- Left Content Area (Task Edit Form) -->
            <div class="flex-1 border-none">
                
                <!-- Header section -->
                <div class="flex justify-between items-center mb-8">
                    <h1 class="text-4xl font-bold text-white tracking-wide">Edit Task</h1>
                ...
                <!-- Edit Form Card -->
                <form method="POST" action="{{ route('tasks.update', $task->id) }}" class="bg-white rounded-2xl p-8 shadow-sm relative">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="bg-gray-50 rounded-xl p-6 mb-8">
                        ...
                        <div class="relative mb-6">
                            <input type="text" name="title" value="{{ old('title', $task->title) }}" placeholder="e.g. Launch New Campaign" class="w-full bg-white border border-gray-200 text-gray-800 rounded-lg p-3 pr-12 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-medium">
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=EBF4FF&color=4F46E5" class="h-6 w-6 rounded-full" alt="User">
                            </div>
                        </div>

                        <!-- Description Input -->
                        <div class="bg-white border text-gray-600 border-gray-200 rounded-lg p-4 mb-6 shadow-sm">
                            <textarea name="description" rows="4" class="w-full bg-transparent border-none p-0 focus:ring-0 resize-none text-sm">{{ old('description', $task->description) }}</textarea>
                        </div>

                        <!-- Status Selection -->
                        @php
                            $currentStatus = old('status', strtolower($task->status?->value ?? $task->status));
                        @endphp
                        <div class="mb-6 flex flex-wrap items-center gap-4">
                            <label class="text-gray-900 font-bold w-20">Status</label>
                            
                            <label class="cursor-pointer">
                                <input type="radio" name="status" value="pending" class="peer sr-only" {{ $currentStatus == 'pending' ? 'checked' : '' }}>
                                <div class="px-5 py-2 rounded-lg font-medium text-sm border bg-white text-gray-500 border-gray-200 peer-checked:bg-gray-200 peer-checked:text-gray-800 peer-checked:border-gray-300 transition-colors">
                                    Pending
                                </div>
                            </label>

                            <label class="cursor-pointer">
                                <input type="radio" name="status" value="in_progress" class="peer sr-only" {{ $currentStatus == 'in_progress' ? 'checked' : '' }}>
                                <div class="px-5 py-2 rounded-lg font-medium text-sm border bg-white text-blue-500 border-gray-200 peer-checked:bg-blue-500 peer-checked:text-white peer-checked:border-blue-500 transition-colors flex items-center gap-2">
                                    + In Progress
                                </div>
                            </label>
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div class="px-5 py-2 rounded-lg font-medium text-sm border bg-white text-green-500 border-gray-200 peer-checked:bg-green-500 peer-checked:text-white peer-checked:border-green-500 transition-colors flex items-center gap-2">
                                    + Completed
                                </div>
                            </label>
                        </div>
                        ...
                        <!-- Priority Selection -->
                        @php
                            $currentPriority = old('priority', strtolower($task->priority?->value ?? $task->priority));
                        @endphp
                        <div class="mb-6 flex flex-wrap items-center gap-4">
                            <label class="text-gray-900 font-bold w-20">Priority</label>
                            
                            <!-- Notice how the image groups them visually but we will use radio buttons styled as chips -->
                            <label class="cursor-pointer">
                                <input type="radio" name="priority" value="low" class="peer sr-only" {{ $currentPriority == 'low' ? 'checked' : '' }}>
                                <div class="px-5 py-2 rounded-lg font-medium text-sm border bg-white text-gray-500 border-gray-200 peer-checked:bg-blue-500 peer-checked:text-white peer-checked:border-blue-500 transition-colors">
                                    Low
                                </div>
                            </label>
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div class="px-5 py-2 rounded-lg font-medium text-sm border bg-white text-gray-500 border-gray-200 peer-checked:bg-yellow-100 peer-checked:text-yellow-700 peer-checked:border-yellow-200 transition-colors flex items-center gap-2">
                                    + Medium
                                </div>
                            </label>
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div class="px-5 py-2 rounded-lg font-medium text-sm border bg-white text-gray-500 border-gray-200 peer-checked:bg-red-100 peer-checked:text-red-700 peer-checked:border-red-200 transition-colors flex items-center gap-2">
                                    + High
                                </div>
                            </label>
                        </div>
                        ...
                        <div class="mb-4 text-gray-700">
                            <div class="relative max-w-md">
                                <input type="date" name="due_date" value="{{ old('due_date', $task->due_date) }}" class="w-full bg-white border border-gray-200 text-gray-600 rounded-lg p-3 pr-10 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative max-w-md">
                                <select name="assigned_to" class="w-full bg-white text-gray-600 border border-gray-200 rounded-lg p-3 appearance-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
